...
from num_boost import Tensor, to_dict
...
def test_C_Tensor_negative_backward(array, grad):
    ...
    torch_grad = torch.tensor(grad)
    with core.enable_track():
        res = jax.jit(result1.backward)(grad)
    result2.backward(torch_grad)
...
@pytest.mark.parametrize("array, grad", [(np.random.randn(2, 5, 3), np.random.randn(2, 5, 3))])
def test_C_Tensor_tensordot_backward(array, grad):
    ...
    with core.enable_track():
        res = jax.jit(result1.backward)(result1.data)
