...
bool shape_isbroadcastable_to_ex(npy_intp *a_shape, npy_intp *b_shape, int a_ndim, int b_ndim, npy_intp **a_new_shape)
{
    ...
    if (a_ndim == b_ndim)
    {
        DEBUG_PRINT("a_ndim == b_ndim\n");
        new_shape = (npy_intp *)malloc(b_ndim * sizeof(npy_intp));
        memcpy(new_shape, a_shape, b_ndim * sizeof(npy_intp));
        bool isbroad = isbroadcastable_same_shape(new_shape, b_shape, a_ndim, a_new_shape, NULL);
        return isbroad;
    }
    else
    {
...
// given a shape (1, 2, 1, 1, 3, 4) we will find the 2 and its corresponding stride
