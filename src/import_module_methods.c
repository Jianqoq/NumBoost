...
_Generic_check_import_if_success(PyObject **struct_with_methods,
                                 const char *message, int start_line) {
...
XLA_OPS *import_xla_ops(XLA_OPS **xla_ops_) {
  ...
  *xla_ops_ = malloc(sizeof(XLA_OPS));
  ...
  xla_ops->and_ = PyObject_GetAttrString(ops, "And");
  xla_ops->approx_topK = PyObject_GetAttrString(ops, "ApproxTopK");
  xla_ops->approx_topK_fallback =
      PyObject_GetAttrString(ops, "ApproxTopKFallback");
  xla_ops->approx_TopK_reduction_output_size =
      PyObject_GetAttrString(ops, "ApproxTopKReductionOutputSize");
  ...
  xla_ops->bit_cast_convert_type =
      PyObject_GetAttrString(ops, "BitcastConvertType");
  ...
  xla_ops->collective_permute =
      PyObject_GetAttrString(ops, "CollectivePermute");
  ...
  xla_ops->convert_element_type =
      PyObject_GetAttrString(ops, "ConvertElementType");
  xla_ops->conv_general_dilated =
      PyObject_GetAttrString(ops, "ConvGeneralDilated");
  ...
  xla_ops->custom_call_with_aliasing =
      PyObject_GetAttrString(ops, "CustomCallWithAliasing");
  xla_ops->custom_call_with_computation =
      PyObject_GetAttrString(ops, "CustomCallWithComputation");
  xla_ops->custom_call_with_layout =
      PyObject_GetAttrString(ops, "CustomCallWithLayout");
  ...
  xla_ops->dynamic_update_slice =
      PyObject_GetAttrString(ops, "DynamicUpdateSlice");
  ...
  xla_ops->not_ = PyObject_GetAttrString(ops, "Not");
  xla_ops->optimization_barrier =
      PyObject_GetAttrString(ops, "OptimizationBarrier");
  xla_ops->or_ = PyObject_GetAttrString(ops, "Or");
  ...
  xla_ops->ProductOfElementaryHouseholderReflectors =
      PyObject_GetAttrString(ops, "ProductOfElementaryHouseholderReflectors");
  ...
  xla_ops->ReduceWindowWithGeneralPadding =
      PyObject_GetAttrString(ops, "ReduceWindowWithGeneralPadding");
  xla_ops->regularized_incomplete_beta =
      PyObject_GetAttrString(ops, "RegularizedIncompleteBeta");
  xla_ops->rem = PyObject_GetAttrString(ops, "Rem");
  xla_ops->remove_dynamic_dimension =
      PyObject_GetAttrString(ops, "RemoveDynamicDimension");
  ...
  xla_ops->select_and_scatter_with_general_padding =
      PyObject_GetAttrString(ops, "SelectAndScatterWithGeneralPadding");
  ...
  xla_ops->shift_right_arithmetic =
      PyObject_GetAttrString(ops, "ShiftRightArithmetic");
  xla_ops->shift_right_logical =
      PyObject_GetAttrString(ops, "ShiftRightLogical");
  ...
  xla_ops->xor_ = PyObject_GetAttrString(ops, "xor");
  xla_ops->zeta = PyObject_GetAttrString(ops, "Zeta");
  Py_DecRef(ops);
  Py_DecRef(xla_client);
  if (_Generic_check_import_if_success(
          (PyObject **)xla_ops_,
          "Failed to import xla ops at import_methods.c line", 39) == NULL)
    return NULL;
  else
    return xla_ops;
}
...
np_method *import_np_methods(np_method **NP_METHOD_) {
  *NP_METHOD_ = malloc(sizeof(np_method));
  ...
  NP_METHOD->where = PyObject_GetAttrString(module, "where");
  Py_DecRef(module);
  if (_Generic_check_import_if_success(
          (PyObject **)NP_METHOD_,
          "Failed to import numpy methods at import_methods.c line",
          __LINE__) == NULL)
    return NULL;
  else {
    return NP_METHOD;
...
jnp_method *import_jnp_methods(jnp_method **JNP_METHOD_) {
  *JNP_METHOD_ = malloc(sizeof(jnp_method));
  ...
  if (module == NULL || module2 == NULL || module3 == NULL)
    return NULL;
  JNP_METHOD->unspecified_value =
      PyObject_GetAttrString(module3, "UnspecifiedValue");
...
void free_xla_ops(XLA_OPS *xla_ops) {
  ...
  Py_DecRef(xla_ops->not_);
  Py_DecRef(xla_ops->optimization_barrier);
  Py_DecRef(xla_ops->or_);
