...
#define wrapper(body, generic_type, in_type, out_type, i, inner_loop_size,     \
                result_data_ptr_, ...)                                         \
  for (i = 0; i < inner_loop_size; i++) {                                      \
    body(generic_type, in_type, out_type, i, result_data_ptr_, __VA_ARGS__)    \
  }

/*=== elementwise & binary core ===*/
#define Universal_Operation_Sequential(in_type, out_type, results,             \
                                       first_result, inner_loop_body, ...)     \
  do {                                                                         \
    Replicate(Ptrs, in_type, __VA_ARGS__);                                     \
    npy_intp _size = PyArray_SIZE(first_result);                               \
    Replicate(Result_Ptrs, out_type,                                           \
              Replicate0_With_Comma(Extract, Remove_Parentheses(results)));    \
    npy_intp i;                                                                \
    _Pragma("omp parallel for schedule(static)") wrapper(                      \
        inner_loop_body, Generic(in_type), in_type, out_type, i, _size,        \
...
#define Universal_Operation(in_type, out_type, results, first_result,          \
                            inner_loop_body, new_shapes, ...)                  \
  do {                                                                         \
    ...
    Replicate(Init_Result_Ptrs, out_type, Remove_Parentheses(results));        \
    for (int i = 0; i < ndim; i++) {                                           \
      shape_cpy[i]--;                                                          \
      shape_copy[i] = 0;                                                       \
    ...
    npy_intp num_threads = outter_loop_size < omp_get_max_threads()            \
                               ? outter_loop_size                              \
                               : omp_get_max_threads();                        \
    Replicate2(Init_Result_Ptrs_Arr, num_threads, out_type,                    \
    ...
    Omp_Parallel(num_threads,                                                  \
      ...
      _Pragma("omp for schedule(static)") for (k = 0; k < outter_loop_size;    \
                                               k++) {                          \
        npy_intp i;                                                            \
        wrapper(inner_loop_body, Generic(in_type), in_type, out_type, i,       \
                inner_loop_size,                                               \
                Replicate0(Get_Result_Ptr, Remove_Parentheses(results)),       \
                Replicate0(Stries_Last, __VA_ARGS__),                          \
                Replicate0(Ptr_Saved, __VA_ARGS__));                           \
...
#define Perform_Universal_Operation(                                           \
    in_type, out_type, in_type_enum, result_type_enum, return_arr,             \
    inner_loop_body, out_array, out_array_len, results, ...)                   \
  do {                                                                         \
    ...
    Replicate_Correct_Type(Correct_Type, in_type_enum, in_type, __VA_ARGS__);  \
...
#define Perform_Reduction_Operation(a, result, axes, axis_len, out, init_val,  \
  ...
  if (out != NULL) {                                                           \
    ...
    if (PyArray_TYPE(out) != result_type_enum) {                               \
      ...
      npy_intp result_mem_size = (npy_intp)result_size * sizeof(in_type);      \
      if (out_mem_size != result_mem_size) {                                   \
        PyErr_SetString(PyExc_ValueError,                                      \
  ...
  if (a_ndim == axis_len) {                                                    \
    ...
    int num_threads =                                                          \
        (int)size < omp_get_max_threads() ? (int)size : omp_get_max_threads(); \
    ...
    if (!is_left) {                                                            \
      ...
      _Pragma("omp parallel num_threads(num_threads)") {                       \
        ...
        _Pragma("omp for schedule(static)") for (p = 0; p < result_size;       \
          ...
          for (npy_intp j = 0; j < inner_loop_size_2; j++) {                   \
            ...
            for (npy_intp h = a_ndim_except_last - 1; h >= 0; h--) {           \
      ...
      npy_intp inner_loop_size_2 = a_size / result_size;                       \
                                                                               \
      int num_threads = (int)outer_loop_size < omp_get_max_threads()           \
                            ? (int)outer_loop_size                             \
                            : omp_get_max_threads();                           \
      npy_intp task_amount = 0;                                                \
      in_type **result_ptr_arr =                                               \
      ...
      _Pragma("omp parallel num_threads(num_threads)") {                       \
        ...
        _Pragma("omp for schedule(static)") for (p2 = 0; p2 < outer_loop_size; \
          ...
          for (npy_intp j = 0; j < inner_loop_size_2; j++) {                   \
            ...
            for (npy_intp h = a_last_index; h >= result_nd; h--) {             \
...
#define Arg_Reduction_Operation(a, result, axes, axis_len, out, init_val,      \
  ...
  _Pragma("omp parallel num_threads(num_threads)") {                           \
    ...
    _Pragma("omp for schedule(static)") for (p = 0; p < result_size; p++) {    \
      Generic(in_type) val = Promote(in_type, init_val);                       \
      for (npy_intp i = 0; i < inner_loop_size; i++) {                         \
    ...
    if (!is_left) {                                                            \
      ...
      _Pragma("omp parallel num_threads(num_threads)") {                       \
        ...
        _Pragma("omp for schedule(static)") for (p = 0; p < result_size;       \
          ...
          result_data_ptr[0] =                                                 \
              Demote(in_type, val / (inner_loop_size * inner_loop_size_2));    \
          result_data_ptr++;                                                   \
        }                                                                      \
      }                                                                        \
      free(progress_init_a_data);                                              \
        ...
        _Pragma("omp for schedule(static)") for (p2 = 0; p2 < outer_loop_size; \
          ...
          for (npy_intp j = 0; j < inner_loop_size_2; j++) {                   \
            for (npy_intp idx = 0; idx < inner_loop_size; idx++) {             \
              ...
              Generic(in_type) val = Promote(in_type, result_data_ptr[idx]);   \
              in_type tmp = Demote(in_type, val + a_val);                      \
              result_data_ptr[idx] = tmp;                                      \
            }                                                                  \
          ...
          for (npy_intp idx = 0; idx < inner_loop_size; idx++) {               \
            Generic(in_type) val = Promote(in_type, result_data_ptr[idx]);     \
            result_data_ptr[idx] = Demote(in_type, val / inner_loop_size_2);   \
          }                                                                    \
          for (npy_intp t = result_nd_except_last - 1; t >= 0; t--) {          \
            if (a_data_progess[t] < transposed_shape_cpy[t]) {                 \
              a_data_progess[t]++;                                             \
