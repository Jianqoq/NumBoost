...
inline npy_float32 half_cast_float(npy_half value) {
  ...
  case 0x7c00u: {
    uint32_t result =
        float16_sign + 0x7f800000u + (((uint32_t)(float16_min)) << 13);
    uint32_t *p = &result;
    npy_float32 P2 = *((npy_float32 *)p);
    return P2;
  }
  default: {
    uint32_t result =
        (float16_sign + (((uint32_t)(value & 0x7fffu) + 0x1c000u) << 13));
    uint32_t *p = &result;
    npy_float32 P2 = *((npy_float32 *)p);
    return P2;
  }
...
inline npy_uint half_cast_uint(npy_half value) {
  ...
  default: {
    ...
    if (exponent < 0) {
      npy_uint result =
          ((((value & 0x03ffu) >> -exponent) >> 10) + (1 >> -exponent)) *
          ((value & 0x8000u) ? -1 : 1);
      return result;
    } else {
      npy_uint result =
...
inline npy_double half_cast_double(npy_half value) {
  ...
  case 0x7c00u: {
    uint64_t result = float16_sign + 0x7ff0000000000000ULL +
                      (((uint64_t)(float16_min)) << 42);
    uint64_t *p = &result;
    npy_float64 P2 = *((npy_float64 *)p);
    return P2;
  }
  default: {
    uint64_t result =
        (float16_sign + (((uint64_t)(value & 0x7fffu) + 0xfc000u) << 42));
    uint64_t *p = &result;
    npy_float64 P2 = *((npy_float64 *)p);
    return P2;
  }
...
#define CAST_ARRAY(source, result, source_type, to_type, npy_enum)             \
  for (i = 0; i < ndims; i++)                                                  \
    new_strides[i] = (npy_intp)((strides[i] * sizeof(to_type)) /               \
                                (npy_intp)sizeof(source_type));                \
  PyArrayObject *array_##source_type = (PyArrayObject *)PyArray_NewFromDescr(  \
      &PyArray_Type, PyArray_DescrFromType(npy_enum), (int)ndims,              \
      (npy_intp const *)shape, (npy_intp const *)new_strides, NULL,            \
  ...
  free(new_strides);                                                           \
  to_type *array_data_##source_type =                                          \
      (to_type *)PyArray_DATA(array_##source_type);                            \
...
#define F_CAST_ARRAY(source, result, source_type, to_type, npy_enum,           \
                     half_cast_func)                                           \
  for (i = 0; i < ndims; i++)                                                  \
    new_strides[i] = (npy_intp)((strides[i] * sizeof(to_type)) /               \
                                (npy_intp)sizeof(source_type));                \
  ...
  source_type *data_##source_type = (source_type *)PyArray_DATA(*source);      \
  _Pragma("omp parallel for") for (i = 0; i < size; i++)                       \
      array_data_##source_type[i] =                                            \
          (to_type)half_cast_func(data_##source_type[i]);                      \
  if (result != NULL)                                                          \
    *result = array_##source_type;                                             \
  else {                                                                       \
    Py_DECREF(*array);                                                         \
...
#define CAST_ARRAY_BOOL(source, result, source_type)                           \
  PyArrayObject *array_##source_type =                                         \
      (PyArrayObject *)PyArray_EMPTY((int)ndims, shape, NPY_BOOL, 0);          \
  if (array_##source_type == NULL) {                                           \
    *result = NULL;                                                            \
    *array = NULL;                                                             \
    return;                                                                    \
  }                                                                            \
  npy_bool *array_data_##source_type =                                         \
      (npy_bool *)PyArray_DATA(array_##source_type);                           \
  source_type *data_##source_type = (source_type *)PyArray_DATA(*source);      \
  _Pragma("omp parallel for") for (i = 0; i < size; i++)                       \
