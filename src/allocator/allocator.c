...
#define Numboost_Parallel_Memset(ptr, size, value) memset(ptr, value, size);
#elif __linux__
...
#define Numboost_Parallel_Memset(ptr, size, value)                             \
  npy_intp __i;                                                                \
  _Pragma("omp parallel for") for (__i = 0; __i < size; __i++) ptr[__i] = value;
#endif
...
/*
Being linked to mem_chain free_partial_mem_blocks function in tensor.c
PyInit_Numboost function
@param self mem_chain
@param s node to be freed partial memory blocks
@param to_free_num number of memory blocks to be freed
*/
void chain_free_partial_mem_blocks(Mem_Chain *self, cache *s,
                                   int32_t to_free_num) {
...
/*
Being linked to mem_chain move_node_to_head function in tensor.c PyInit_Numboost
function
@param self mem_chain
...
It is designed to manage a hash table called cache_pool and a doubly-linked list
named mem_chain. These structures are used for caching and reusing previously
allocated memory blocks. Numboost try its best to avoid calling malloc and free
functions especially free since free() is a very expensive operation. The
default_malloc function will first check if there is any available memory in the
cache_pool. If there is, it will retrieve the memory from the cache_pool.
Otherwise, it will call malloc to allocate memory and create a cache_pool for
it.
...
allocated memory blocks. Numboost try its best to avoid calling malloc and free
functions especially free since free() is a very expensive operation. The
default_free function will first check if there is available memory based on
thread hold in the system. If there is, it will cache the memory in the
cache_pool. Otherwise, it will try to free the least recently used memory block.
Numboost will try to free enough memory which is greater or equal than the size
of the memory to be cached.
@param ctx unsed, but must be passed to the default_malloc function
...
static void default_free(void *ctx, void *ptr, size_t size) {
  ...
  uint64_t predict_possible_cache_size =
      mem_chain->max_possible_cache_size + size * s->max_mem * 2;
  ...
  if (s->mem_allocated >= 0 /*has availabel mem*/
    ...
    {
    /*first we need to let to_free ptr points the the node which have available
     * memory to free*/
    ...
    if (to_free->mem_allocated < 0) {
      ...
      if (to_free->mem_allocated <
          0) /*If this condition == true, to_free will be head of mem_chain and
                no memory we can free from the mem_chain*/
      {
        free(ptr);
        return;
      }
    }
    /*size to cache is less than the total cached size in tail node, we can free
     * all or partial of the memory in that node*/
    if (size <= to_free->tensor_size * (to_free->mem_allocated + 1)) {
      int32_t to_free_num = (size % to_free->tensor_size) == 0
                                ? size / to_free->tensor_size
                                : size / to_free->tensor_size + 1;
      mem_chain->free_partial_mem_blocks(mem_chain, to_free, to_free_num);
      mem_chain->max_possible_cache_size +=
    ...
    } else /*We need to free memory not only in one node, but multiple*/
    {
      ...
      /*When this condition is true, which means the cached memory in the memory
       * pool is relative small compare with the current memory we want to
       * cache*/
      if (mem_chain->tail == mem_chain->head) {
        assert(mem_chain->head ==
               to_free); // to_free should be the head of the mem_chain,
                         // otherwise, there is a bug in the code
        if (size <= to_free->tensor_size * (to_free->mem_allocated + 1) &&
            to_free->mem_allocated >= 0) {
          int32_t to_free_num = (size % to_free->tensor_size) == 0
                                    ? size / to_free->tensor_size
                                    : size / to_free->tensor_size + 1;
          mem_chain->free_partial_mem_blocks(mem_chain, to_free, to_free_num);
...
PyDataMem_Handler my_handler = {"mem_handler", 1,
                                .allocator = {
                                    .ctx =
                                        &(PyDataMem_Funcs){
