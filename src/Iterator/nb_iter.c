...
static int tensor_iter_traverse(TensorIteratorObject *self, visitproc visit,
                                void *arg) {
...
TensorIteratorObject *iterator_new(PyTypeObject *type, Tensor *self) {
  TensorIteratorObject *iter =
      (TensorIteratorObject *)PyObject_GC_New(TensorIteratorObject, type);
  if (iter == NULL) {
    return NULL;
  }
