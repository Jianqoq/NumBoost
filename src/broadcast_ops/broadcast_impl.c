...
void Broad_Cast_vec_(PyArrayObject *a, PyArrayObject *b,
                     PyObject **array_result) {
  ...
  if (a_ndim < b_ndim) {
    ...
    bigger = b;
    ndim = PyArray_NDIM(b);
    if (!shape_isbroadcastable_to_ex(a_shape, b_shape, a_ndim, b_ndim,
                                     &to_broadcast_shape_pad_one)) {
    ...
    bigger = a;
    ndim = PyArray_NDIM(a);
    if (!shape_isbroadcastable_to_ex(b_shape, a_shape, b_ndim, a_ndim,
                                     &to_broadcast_shape_pad_one)) {
      PyErr_SetString(PyExc_ValueError, "Cannot broadcast shapes");
      return;
    }
  ...
  npy_intp stride_last =
      PyArray_STRIDE((const PyArrayObject *)bigger, ndim - 1);
  npy_intp *strides_a = NULL, *strides_b = NULL, *shape = NULL;
  npy_intp *shape_copy = malloc(sizeof(npy_intp) * ndim);
  bool done = false;

  predict_broadcast_shape(to_broadcast_shape_pad_one, bigger_shape, ndim,
                          &shape);
  ...
  PyArrayObject *result = (PyArrayObject *)PyArray_EMPTY(
      ndim, (npy_intp const *)shape, NPY_INT32, 0);
  if (shape_copy == NULL) {
  ...
  npy_intp total_size =
      find_innerloop_size(bigger_shape, to_broadcast_shape_pad_one, ndim);
...
PyArrayObject *numboost_broadcast(PyArrayObject *a, PyArrayObject *b,
                                  int enum_op) {
  ...
  int result_type =
      binary_result_type(enum_op, ((PyArrayObject_fields *)a)->descr->type_num,
                         ((PyArrayObject_fields *)a)->descr->elsize,
                         ((PyArrayObject_fields *)b)->descr->type_num,
