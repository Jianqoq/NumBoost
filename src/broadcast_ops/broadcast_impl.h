...
#define Standard_Inner_Loop(type, op, inner_loop_size, stride_a, stride_b,     \
...
#define Shift_Inner_Loop(type, op, inner_loop_size, stride_a, stride_b, a_ptr, \
                         b_ptr, result_ptr)                                    \
  {                                                                            \
    for (int i = 0; i < inner_loop_size; i++) {                                \
      type val1 = *(a_ptr + i * stride_a);                                     \
...
#define Half_Inner_Loop(type, op, inner_loop_size, stride_a, stride_b, a_ptr,  \
                        b_ptr, result_ptr)                                     \
  {                                                                            \
    for (int i = 0; i < inner_loop_size; i++) {                                \
...
#define Pow_Inner_Loop(type, op, inner_loop_size, stride_a, stride_b, a_ptr,   \
                       b_ptr, result_ptr)                                      \
  {                                                                            \
    for (int i = 0; i < inner_loop_size; i++) {                                \
      type val1 = *(a_ptr + i * stride_a);                                     \
...
#define Powf_Inner_Loop(type, op, inner_loop_size, stride_a, stride_b, a_ptr,  \
                        b_ptr, result_ptr)                                     \
  {                                                                            \
    for (int i = 0; i < inner_loop_size; i++) {                                \
      type val1 = *(a_ptr + i * stride_a);                                     \
...
#define Powl_Inner_Loop(type, op, inner_loop_size, stride_a, stride_b, a_ptr,  \
                        b_ptr, result_ptr)                                     \
  {                                                                            \
    for (int i = 0; i < inner_loop_size; i++) {                                \
      type val1 = *(a_ptr + i * stride_a);                                     \
...
#define NestedLoop_Maintainer(last_index, current_shape_process, shape, a_ptr, \
                              b_ptr, indice_a_cache, indice_b_cache)           \
...
#define Prepare_For_Parallel(outer_loop_size, inner_loop_size, a_strides,      \
                             b_strides, shape, result_ptr1, result_ptr2,       \
                             result_ptr3)                                      \
  int thread_id = omp_get_thread_num();                                        \
  npy_intp start_index = thread_id * (outer_loop_size / num_threads) +         \
                         min(thread_id, outer_loop_size % num_threads);        \
  npy_intp end_index = start_index + outer_loop_size / num_threads +           \
                       (thread_id < outer_loop_size % num_threads ? 1 : 0);    \
...
#define Fill_Shape_and_Indice(shape_, shape_copy_, indice_a_, indice_b_,       \
                              strides_a_, strides_b_, ndim)                    \
  for (int i = 0; i < ndim; i++) {                                             \
    shape_[i]--;                                                               \
    shape_copy_[i] = 0;                                                        \
...
#define Register_Broadcast_Operation(type, suffix, op, inner_loop_body)        \
  static PyArrayObject *Broadcast_Standard_##type##_##suffix(                  \
    ...
    if (a_ndim < b_ndim) {                                                     \
      ...
      if (!shape_isbroadcastable_to_ex(PyArray_SHAPE(a_), b_shape, a_ndim,     \
                                       b_ndim, &a_shape)) {                    \
      ...
      if (!shape_isbroadcastable_to_ex(PyArray_SHAPE(b_), a_shape, b_ndim,     \
                                       a_ndim, &b_shape)) {                    \
        PyErr_SetString(PyExc_ValueError, "Cannot broadcast shapes");          \
        return NULL;                                                           \
      }                                                                        \
    ...
    npy_##type *b_data_ptr_saved =                                             \
        (npy_##type *)PyArray_DATA((PyArrayObject *)b_);                       \
    npy_##type *a_data_ptr_saved =                                             \
        (npy_##type *)PyArray_DATA((PyArrayObject *)a_);                       \
    npy_##type *result_data_ptr_ = (npy_##type *)PyArray_DATA(result);         \
    npy_##type *result_data_ptr_cpy = (npy_##type *)PyArray_DATA(result);      \
    for (int i = 0; i < ndim; i++) {                                           \
    ...
    npy_intp num_threads = outer_loop_size < omp_get_max_threads()             \
                               ? outer_loop_size                               \
                               : omp_get_max_threads();                        \
    npy_##type **result_ptr_ =                                                 \
        (npy_##type **)malloc(sizeof(npy_##type *) * num_threads);             \
    ...
    for (int id = 0; id < num_threads; id++) {                                 \
      ...
      npy_intp prd = result_ptr_[id] - (npy_##type *)PyArray_DATA(result);     \
      npy_intp *current_shape_process =                                        \
          (npy_intp *)calloc(ndim, sizeof(npy_intp));                          \
      for (int j = max_dim; j >= 0; j--) {                                     \
        current_shape_process[j] = prd % (shape[j] + 1);                       \
    ...
    _Pragma(Str(omp parallel num_threads(num_threads) firstprivate(            \
      ...
      _Pragma("omp for schedule(static)") for (k = 0; k < outer_loop_size;     \
                                               k++) {                          \
        inner_loop_body(npy_##type, op, inner_loop_size, stride_a_last,        \
